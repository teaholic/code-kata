...
class NumeralGateway:
    def __init__(self, number: int, numeral_sequence: List[int]):
        ...
        self.residues = SubtractionService(number=number, numeral_sequence=numeral_sequence).run()
    ...
    def get_closest_smaller_number(self) -> int:
        ...
        return [num for pos, num in self.enumerated_numeral_sequence if pos == closest_smaller_residue_index][0]

    def get_closest_greater_number(self) -> int:
        closest_greater_residue_index = self.residues.index(self.get_closest_greater_residue())
        return [num for pos, num in self.enumerated_numeral_sequence if pos == closest_greater_residue_index][0]
    ...
    def get_closest_smaller_tenth(self) -> int:
        ...
        return [num for pos, num in self.enumerated_numeral_sequence if pos == closest_smaller_tenth_index][0]
