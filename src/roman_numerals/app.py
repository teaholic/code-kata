from src.roman_numerals.model import RomanToArabicMappingService, NumeralService, RomanToArabicMapping
...
class NumeralApp:
    ...
    def convert(self, number:int) -> str:
        closest_greater_number_residue = NumeralService(number, [n.value for n in RomanToArabicMapping]).get_closest_greater_residue()
        if closest_greater_number_residue == 0:
            return self.mapping_service.get_roman_numeral(number)
        else:
            closest_smaller_number = NumeralService(number, [n.value for n in RomanToArabicMapping]).get_closest_smaller_number()
            closest_greater_number = NumeralService(number, [n.value for n in RomanToArabicMapping]).get_closest_greater_number()
            if (number % closest_smaller_number == 0) & (closest_greater_number + closest_greater_number_residue != number):#number < 4:
                ...
                if closest_greater_number_residue == - NumeralService(number, [n.value for n in RomanToArabicMapping]).get_closest_smaller_tenth():
                    if closest_smaller_number == NumeralService(number, [n.value for n in RomanToArabicMapping]).get_closest_smaller_tenth():
